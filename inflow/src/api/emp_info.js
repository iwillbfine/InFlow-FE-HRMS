...
export const getHomeInfo = async (employeeId, month) => {
  try {
      const response = await apiClient.get(`/home?employeeId=${employeeId}&month=${month}`);
...
export const getEmploymentContract = async (employeeId, token) => {
  try {
    const response = await apiClient.get(`/employees/${employeeId}/employment-contract`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
export const getSecurityAgreement = async (employeeId, token) => {
  try {
    const response = await apiClient.get(`/employees/${employeeId}/security-agreement`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

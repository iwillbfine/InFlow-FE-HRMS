        ...
        <TableItem gtc="5fr 20fr" style="height: 50%;" v-else>
            ...
            <TableRow>
                ...
                <TableCell class="h-7 pl-2" fs="1.6rem" botr>
                    <div v-if="isEditMode">
                        ...
                        <option v-for="dept in upperDepartments" :key="dept.department_code" :value="dept.department_name">
            ...
            <TableRow>
                ...
                <TableCell class="h-7 pl-2" fs="1.6rem" botr>
                    ...
                    <div v-else>
                        <span>
                        {{ departmentInfo.min_employee_num || '정보가 없습니다.' }}
                        </span>명
                    </div>
                </TableCell>
            </TableRow>
...
<script setup>
    ...
    const fetchDepartmentInfo = async (departmentCode) => {
        try {
            const response = await apiClient.get(`/departments/detail/department-code/${departmentCode}`);
            departmentInfo.value = response.data.content[0];
            // editableData 초기화
            editableData.value = { ...departmentInfo.value };
            console.log("선택된 부서 정보:", departmentInfo.value);
        } catch (error) {

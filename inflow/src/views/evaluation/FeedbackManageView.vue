<template>
    <SectionItem class="payment-section" h="100%" w="calc(100% - 52rem)">
      <CommonArticle label="피드백 등록 및 수정">
      ...
      <CommonArticle label="피드백 내역">
        <FlexItem
          class="created-feedback"
          fld="column"
          w="100%"
          p="2rem"
          bgc="#fff"
          ...
          mb="2rem"
        >
          <span v-if="feedbackData && feedbackData.content" class="feedback-content">
            {{ feedbackData.content }}
          </span>
  
          <span v-else class="feedback-content center-message">
            피드백이 존재하지 않습니다.
          </span>
        </FlexItem>
      </CommonArticle>
  ...
  // 피드백 조회
  watch([selectedEmployeeId, selectedYear, selectedHalf], async (newValues) => {
  ...
  const handleOnclick = async () => {
    if (!selectedEmployeeId.value || !selectedYear.value || !selectedHalf.value || !feedbackContent.value.trim()) {
      alert('모든 필드를 입력해주세요.');
      return;
    }

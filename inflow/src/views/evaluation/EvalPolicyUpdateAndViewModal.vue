<template>
  ...
  import { findEvaluationPolicyById, updateEvaluationPolicy } from '@/api/evaluation';
  ...
  const handleDateUpdate = (field, date) => {
    if (field === 'modifiable_date') {
      ...
      editForm.value[field] = `${date}T${field === 'end_date' ? '23:59:59' : '00:00:00'}`;
    }
  };
  ...
  const handleUpdate = async () => {
    try {
      const response = await updateEvaluationPolicy(props.policyId, editForm.value);
      if (response.success) {
        ...
        alert(`정책 수정 실패: ${response.message || '알 수 없는 오류가 발생했습니다.'}`);
      }
    } catch (error) {

...
import { getReturnRequestsByEmployeeId } from '@/api/attendance';
...
const fetchReturnRequestData = async (eid, page, date) => {
  const response = await getReturnRequestsByEmployeeId(eid, page, date);
  if (response.success) {
...
const handleChangePage = (page) => {
  curPage.value = page;
  router.push({
    path: '/hr-basic/attendance/return/requests',
...
const goBack = () => {
  router.push('/hr-basic/attendance/return');
};
...
watch(
  ...
  (newQuery) => {
    ...
    fetchReturnRequestData(eid.value, curPage.value, curMonth.value);
  },
  { immediate: true }
);

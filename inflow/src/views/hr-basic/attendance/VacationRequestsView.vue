    ...
    <div class="table-wrapper">
      <TableItem gtc="1fr 3fr 3fr 1.5fr 1fr 1.25fr">
        ...
        <TableRow
          ...
          :key="index"
        >
          <TableCell class="mid" fs="1.6rem" :botl="index === vacationRequestList.length - 1">{{
          ...
          <TableCell class="mid" fs="1.6rem" :botr="index === vacationRequestList.length - 1">
...
import { getVacationRequestsByEmployeeId } from '@/api/vacation';
...
const fetchVacationRequestData = async (eid, page, date) => {
  const response = await getVacationRequestsByEmployeeId(eid, page, date);
  if (response.success) {
    vacationRequestList.value = response.content.elements;
...
const handleChangePage = (page) => {
  curPage.value = page;
  router.push({
    path: '/hr-basic/attendance/vacation/requests',
...
const goBack = () => {
  router.push('/hr-basic/attendance/vacation');
};
...
watch(
  ...
  (newQuery) => {
    ...
    fetchVacationRequestData(eid.value, curPage.value, curMonth.value);
  },
  { immediate: true }
);

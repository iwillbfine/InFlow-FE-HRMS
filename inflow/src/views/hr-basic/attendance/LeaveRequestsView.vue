    ...
    <div class="table-wrapper">
      <TableItem gtc="1fr 3fr 3fr 1.5fr 1fr 1.25fr">
        ...
        <TableRow
          ...
          :key="index"
        >
          <TableCell class="mid" fs="1.6rem" :botl="index === leaveRequestList.length - 1">{{
          ...
          <TableCell class="mid" fs="1.6rem" :botr="index === leaveRequestList.length - 1">
...
const fetchLeaveRequestData = async (eid, page, date) => {
  const response = await getLeaveRequestsByEmployeeId(eid, page, date);
  if (response.success) {
...
const handleChangePage = (page) => {
  curPage.value = page;
  router.push({
    path: '/hr-basic/attendance/leave/requests',
...
const goBack = () => {
  router.push('/hr-basic/attendance/leave');
};
...
watch(
  ...
  (newQuery) => {
    ...
    fetchLeaveRequestData(eid.value, curPage.value, curMonth.value);
  },
  { immediate: true }
);

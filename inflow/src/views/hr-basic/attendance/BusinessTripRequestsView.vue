    ...
    <div class="table-wrapper">
      <TableItem gtc="1fr 1.5fr 3fr 3fr 1.5fr 1fr 1.25fr">
        <TableRow>
          ...
          <TableCell th fs="1.6rem">출장지</TableCell>
        ...
        <TableRow v-if="!isEmpty" v-for="(item, index) in businessTripRequestList" :key="index">
          <TableCell class="mid" fs="1.6rem" :botl="index === businessTripRequestList.length-1">{{ item.attendance_request_id }}</TableCell>
          ...
          <TableCell class="mid" fs="1.6rem" :botr="index === businessTripRequestList.length-1">
...
import { getBusinessTripRequestsByEmployeeId } from '@/api/attendance';
...
const fetchBusinessTripRequestData = async (eid, page, date) => {
  const response = await getBusinessTripRequestsByEmployeeId(eid, page, date);
  if (response.success) {
...
const handleChangePage = (page) => {
  curPage.value = page;
  router.push({ path: '/hr-basic/attendance/business-trip/requests', query: { page: curPage.value, date: curMonth.value } });
}

const goPrevMonth = (prevMonth) => {
  curPage.value = 1;
  router.push({ path: '/hr-basic/attendance/business-trip/requests', query: { page: curPage.value, date: prevMonth } });
}

const goNextMonth = (nextMonth) => {
  curPage.value = 1;
  router.push({ path: '/hr-basic/attendance/business-trip/requests', query: { page: curPage.value, date: nextMonth } });
}

const goSelectedMonth = (selectedMonth) => {
  curPage.value = 1;
  router.push({ path: '/hr-basic/attendance/business-trip/requests', query: { page: curPage.value, date: selectedMonth } })
}

const goBack = () => {
  router.push('/hr-basic/attendance/business-trip');
}
...
watch(
  ...
  (newQuery) => {
    ...
    fetchBusinessTripRequestData(eid.value, curPage.value, curMonth.value)
  },
  { immediate: true }
)
